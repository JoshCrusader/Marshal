...
public class HomememberDAO {
    ...
    public static Homemember GetHomeMember(String userId){
        Connection conn = DatabaseUtils.retrieveConnection();
        Homemember owner = null;
        try{
            String sql = "SELECT * FROM USERS U RIGHT JOIN HOMEMEMBER HM ON HM.userID = U.userID where U.userID = '"+userId+"';";
            PreparedStatement pStmt = conn.prepareStatement(sql);
            ResultSet rs = pStmt.executeQuery();
            while(rs.next()){
                owner = new Homemember(rs.getString(1), rs.getString(2),rs.getInt(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getInt(8),rs.getInt(9),rs.getString(10),rs.getInt(11),rs.getInt(12),rs.getString(13),rs.getString(14),rs.getString(15),rs.getBoolean(17),rs.getInt(18),rs.getInt(19));
